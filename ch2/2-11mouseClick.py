...
def draw(event,x,y,flags,param):		# 마우스 콜백 함수
    ...
    if event==cv2.EVENT_LBUTTONDOWN:	# 마우스 왼쪽 버튼 클릭했을 때
       if flags&cv2.EVENT_FLAG_SHIFTKEY:  # Shift키를 눌렀을 떄 -> 만약 안눌리면 그림 안그려지지><
            cv2.rectangle(img,(x,y),(x+200,y+200),(0,0,255),2) # 빨간색
    elif event==cv2.EVENT_RBUTTONDOWN:	# 마우스 오른쪽 버튼 클릭했을 때
        cv2.rectangle(img,(x,y),(x+100,y+100),(255,0,0),2) # 파란색
