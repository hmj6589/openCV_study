...
def painting(event,x,y,flags,param):
    if event==cv2.EVENT_LBUTTONDOWN:
        ...
        cv2.circle(mask,(x,y),BrushSiz,cv2.GC_FGD,-1)   # mask에 사용자가 GC_FGD(물체)을 넣겠다
    elif event==cv2.EVENT_RBUTTONDOWN:
        cv2.circle(img_show,(x,y),BrushSiz,RColor,-1)	# 오른쪽 버튼 클릭하면 빨간색
        cv2.circle(mask,(x,y),BrushSiz,cv2.GC_BGD,-1)   # mask에 사용자가 GC_BGD(배경)을 넣겠다
...
grab=img*mask2[:,:,np.newaxis]  # if mask2가 0(배경)이면 0, 아니면(1이면) 자기 자신의 색상 그대로
