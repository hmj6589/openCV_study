...
# 2개의 이미지 가져오기
img1 = cv2.imread('face_images/ggg.png')
if img1 is None:
    print("이미지를 불러올 수 없습니다. 파일 경로를 확인해주세요.")

scale_factor = 0.5  # 이미지 크기를 50%로 축소
img1 = cv2.resize(img1, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_AREA)

img2 = cv2.imread('face_images/kkk.png')
if img2 is None:
    print("이미지를 불러올 수 없습니다. 파일 경로를 확인해주세요.")

scale_factor = 0.5  # 이미지 크기를 50%로 축소
img2 = cv2.resize(img2, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_AREA)
...
for j in range(0, num_images):
    ...
    if key == ord('a'):
        cv2.imwrite('result_face/morphing_face(1).png', morphed_frame)  # 이미지 캡처 및 저장
    elif key == ord('b'):
        cv2.imwrite('result_face/morphing_face(2).png', morphed_frame)  # 이미지 캡처 및 저장
    elif key == ord('c'):
        cv2.imwrite('result_face/morphing_face(3).png', morphed_frame)  # 이미지 캡처 및 저장
    elif key == ord('d'):
        cv2.imwrite('result_face/morphing_face(4).png', morphed_frame)  # 이미지 캡처 및 저장
    elif key == ord('e'):
        cv2.imwrite('result_face/morphing_face(5).png', morphed_frame)  # 이미지 캡처 및 저장
    elif key == ord('f'):
        cv2.imwrite('result_face/morphing_face(6).png', morphed_frame)  # 이미지 캡처 및 저장
    elif key == ord('g'):
        cv2.imwrite('result_face/morphing_face(7).png', morphed_frame)  # 이미지 캡처 및 저장
    elif key == ord('h'):
        cv2.imwrite('result_face/morphing_face(8).png', morphed_frame)  # 이미지 캡처 및 저장
    elif key == ord('i'):
        cv2.imwrite('result_face/morphing_face(9).png', morphed_frame)  # 이미지 캡처 및 저장
    elif key == ord('j'):
        cv2.imwrite('result_face/morphing_face(10).png', morphed_frame)  # 이미지 캡처 및 저장
